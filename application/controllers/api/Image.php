...
class Image extends REST_Controller
{
    ...
    public function editImage_post($id = null)
    {
        // $id = $this->put('id');
    ...
    public function deleteImage_get($id = null)
    {
        $delete = $this->image->delete($id);
        // Check if the image data is deleted
        if ($delete) {
            // Set the response and exit
            $this->response([
                'status' => TRUE,
                'message' => 'The image info has been deleted successfully.'
            ], REST_Controller::HTTP_OK);
        } else {
            // Set the response and exit
            $this->response("Some problems occurred, please try again.", REST_Controller::HTTP_BAD_REQUEST);
